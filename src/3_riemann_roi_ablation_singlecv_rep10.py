...
Overview:
    ...
    METHODOLOGICAL UPDATE: Employs a 10-Times Repeated 5-Fold CV strategy 
    to strictly replicate Li et al. (2026) and ensure a fair comparison with 
    the Euclidean SVM pipeline.
...
def run_ablation():
    ...
    print(f"STARTING SCRIPT 3: ABLATION ({channel_count} CHANNELS - {LAYOUT_NAME}) - 10x REPEATED CV MODE")
    ...
    for band_name in best_bands:
        ...
        for p_name in architectures:
            ...
            print(f"Evaluating Architecture: {p_name} via 10x Repeated CV...")
            ...
            # --- 10-TIMES REPEATED CV IMPLEMENTATION ---
            fe_steps.append(('svm', SVC(class_weight='balanced', random_state=RANDOM_STATE)))
            full_pipeline = Pipeline(fe_steps)
            
            # Genereer handmatig de 10 repeats van 5-Fold Stratified Group CV
            cv_repeated = []
            for i in range(10):
                sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=RANDOM_STATE + i)
                cv_repeated.extend(list(sgkf.split(X_input, y, groups=groups)))
            
            # GridSearchCV zal nu 50 iteraties per parameter-combinatie draaien (10 repeats * 5 folds)
    ...
    report_text = "====================================================\n"
    report_text += f" FINAL {LAYOUT_NAME} ABLATION RESULTS (10x REPEATED CV) \n"
    report_text += "====================================================\n\n"
