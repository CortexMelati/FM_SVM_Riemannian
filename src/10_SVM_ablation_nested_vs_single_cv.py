...
from sklearn.pipeline import make_pipeline # <-- IMPORT TOEGEVOEGD
from sklearn.metrics import balanced_accuracy_score # <-- IMPORT NAAR BOVEN VERPLAATST
...
def run_cv_ablation_study():
    ...
    # 1. PIPELINE & PARAMETERS AANMAKEN
    # 'svc__' prefix for pipeline
    param_grid = {
    ...
    for band_name in BANDS.keys():
        ...
        if not features_path.exists():
            print(f"Skipping {band_name.upper()}: Geen mSFFS features gevonden.")
            continue
        ...
        clf_single = GridSearchCV(
            estimator=pipeline, param_grid=param_grid, cv=cv_inner, 
            scoring='balanced_accuracy', n_jobs=-1
        )
        clf_single.fit(X, y, groups=groups) 
        ...
        for train_idx, test_idx in cv_outer.split(X, y, groups=groups):
            # Splitsing op ongeschaalde data
