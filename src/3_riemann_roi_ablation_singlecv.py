"""
=============================================================================
3. RIEMANNIAN ABLATION (ROI / WHOLE BRAIN SWITCH) - SINGLE CV
=============================================================================
Overview:
    ...
    METHODOLOGICAL UPDATE: Employs Single CV (GridSearchCV directly on the 
    full source training set) to strictly replicate Li et al. (2026) and 
    ensure a fair comparison with the Euclidean SVM pipeline.
...
def run_ablation():
    ...
    print(f"STARTING SCRIPT 3: ABLATION ({channel_count} CHANNELS - {LAYOUT_NAME}) - SINGLE CV MODE")
    ...
    for band_name in best_bands:
        ...
        for p_name in architectures:
            ...
            print(f"Evaluating Architecture: {p_name} via Single CV...")
            ...
            # --- SINGLE CV IMPLEMENTATION ---
            # Koppel de SVM direct aan de pipeline zodat GridSearchCV de hele keten kan testen
            fe_steps.append(('svm', SVC(class_weight='balanced', random_state=RANDOM_STATE)))
            full_pipeline = Pipeline(fe_steps)
            
            # 5-Fold Stratified Group CV (Single level)
            cv_single = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=RANDOM_STATE)
            
            search = GridSearchCV(full_pipeline, svm_param_grid, cv=cv_single, scoring='balanced_accuracy', n_jobs=-1, verbose=1)
            ...
            # Opschonen van de parameter log voor de CSV (verwijder de 'svm__' prefix)
            ...
            # Freezing the winning model direct na de fit
            layout_str = 'whole' if RUN_AS_WHOLE_BRAIN else 'roi'
            best_name = f"model_riemann_{band_name}_{layout_str}_{p_name}.pkl"
            
            # We vervangen de iteratieve opslag door de best_estimator_ direct op te slaan
            # We zetten 'probability=True' aan op het final model in de pipeline voor latere AUC evaluaties
            final_pipe = search.best_estimator_
            final_pipe.named_steps['svm'].probability = True 
            # Fit hem nog één keer met probability=True (nodig voor predict_proba in script 5)
...
# =========================================================================
    ...
    report_text = "====================================================\n"
    report_text += f" FINAL {LAYOUT_NAME} ABLATION RESULTS (SINGLE CV) \n"
    report_text += "====================================================\n\n"
    
    for band_name in best_bands:
        # GEFIXT: Sorteer nu op 'CV_Balanced_Accuracy_Mean'
        ...
        # GEFIXT: Haal de Mean op voor het printen
    ...
    if not df_existing.empty:
        # Verwijder oude runs voor deze layout/bands om duplicaten te voorkomen
        df_existing = df_existing[~((df_existing['Layout'] == LAYOUT_NAME) & (df_existing['Band'].isin([b.upper() for b in best_bands])))]
        
        # Voorkom schema-fouten als het oude scoreboard de nieuwe kolommen nog niet had
