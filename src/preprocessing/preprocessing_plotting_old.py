...
def get_plots(raw: mne.io.Raw, step: str, 
    ...
    def plot_psd_img(raw, xscale):
        # max limit for the file
        nyquist = raw.info['sfreq'] / 2.0
        
        # max 60 Hz
        # cannot go above the nyquist frequence
        safe_fmax = min(60.0, nyquist - 0.5) 
        
        # fmax 
        fig = raw.compute_psd(fmin=1, fmax=safe_fmax).plot(picks='eeg', show=False)
        # ----------------------
    ...
    fig.suptitle(f"Report: {step}", fontsize=20)
    plt.tight_layout()
